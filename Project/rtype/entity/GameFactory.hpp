...
namespace rtype
{
    ...
    public:
        ...
        static void createSpaceShip(Entity::ID spaceID,
        ...
        static void createPlayerSpaceShip(Args &&...args) noexcept
        {
            Entity::ID spaceID = _ettMgr->createEntity();
            auto &ettMgr = *_ettMgr;
            ettMgr[spaceID].addComponent<rtc::Player>();
            createSpaceShip(spaceID, std::forward<Args>(args)...);
        }

        static void createBullet(sf::Texture &texture, const sf::FloatRect &AABB) noexcept
        {
