...
namespace rtype
{
    ...
    void Configuration::__fillMultiAnim(const rapidjson::Value &value, Map &map, AnimMap &animMap)
    {
        ...
        for (const auto &cur : value.GetObject()) {
            ...
            for (const auto &val : cur.value.GetArray()) {
                ...
                multiAnimTab.emplace_back(AnimatedSprite(&animations.get(realValue), 
                    animInfo.status, sf::seconds(animInfo.speed), animInfo.loop, animInfo.repeat));
            }
    ...
    void Configuration::__parseCreateAnims()
    {
        ...
        rapidjson::Document doc;
        __loadJSON(path, doc);
        const auto &value = doc["createanim"];
