...
namespace rtype
{
    ...
    public:
        ...
        void connect(const tcp::endpoint &endpoint) noexcept
        {
        ...
        void processPlayer(const boost::system::error_code &error)
        {
            if (!error) {
                ...
                if (std::holds_alternative<matchmaking::PlayerInfo>(packet)) {
                    auto &&player = std::get<matchmaking::PlayerInfo>(packet);
        ...
        void matchFound(const boost::system::error_code &error)
        {
            if (!error) {
                auto packet = _protoConnect->pop();
                if (std::holds_alternative<matchmaking::MatchFound>(packet)) {
                    {
    ...
    private:
        ...
        std::mutex _mutex;
        sf::Sprite _bg;
        std::unique_ptr<asio::io_service> _io{std::make_unique<asio::io_service>()};
