...
namespace rtype
{
    ...
    private:
        bool __setGUI() noexcept
        {
            ...
            return GUIManager::setGUI<LoadingWidgets, nbWidgets>(loadingLayout, _gui, _log);
        }
        ...
        void __configure() noexcept
        {
            auto start = __setGUI();
            if (start) {
                static_cast<CEGUI::ProgressBar &>(_gui[LoadingWidgets::LoadingProgressBar]).setProgress(_progress);
                ...
                _log(lg::Info) << "Number of files to load : " << _nbFiles << std::endl;
                Configuration::initialize(_currentLoaded);
            }
        }
    ...
    public:
        ...
        void update([[maybe_unused]] double timeSinceLastFrame) noexcept final
        {
            if (_isLoading) {
                ...
                static_cast<CEGUI::ProgressBar &>(_gui[LoadingWidgets::LoadingProgressBar]).setProgress(_progress);
                _gui[LoadingWidgets::Percentage].setText(str);
