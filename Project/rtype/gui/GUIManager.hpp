...
namespace rtype
{
    ...
    {
    public:
        GUIManager(gutils::EventManager &evtMgr, sf::RenderWindow &win, const std::string &root);
    ...
    public:
        ...
        static void applyCEGUIFunc(Callback &&cb, FirstArg &&arg,
                                   OtherArgs &&...args) noexcept(noexcept(cb(std::forward<FirstArg>(arg))))
        {
        ...
        {
            return std::all_of(arr.begin(), arr.end(), [](auto &&cur) { return cur != nullptr; });
        }
        ...
        static bool setGUI(const std::string &layout, BaseGUI<Size> &gui, logging::Logger &log) noexcept
        {
            gui.sheet = setSheet(layout);
            if (gui.sheet == nullptr) {
            ...
            bool res = GUIManager::checkWidgets<Size>(gui.widgets);
            if (!res) {
