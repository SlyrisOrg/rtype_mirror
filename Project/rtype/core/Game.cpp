...
namespace rtype
{
    ...
    {
        _systemMgr.get<SVideo>(Sys::Video).draw();
        _systemMgr.get<SScenes>(Sys::Scene).draw();
        _systemMgr.get<SGui>(Sys::Gui).draw();
    }
    ...
    void Game::__launch() noexcept
    {
        ...
        while (!_isRunning) {
            ...
            _log(lg::Debug) << TimePerFrame.asSeconds() << std::endl;
            if (repaint)
                __render();
        }
    }
